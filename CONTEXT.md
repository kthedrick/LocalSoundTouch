...
- **Source-based grouping routing:** `AppLogic.chooseJoinStrategy` decides `ma` / `bose-zone` / `none`. MA grouping (`group-include`/`group-remove`) is used when a **Bose** leader source = AIRPLAY, **or** when a **WiiM** leader is actively playing. A WiiM leader never reports AIRPLAY — MA's native `wiim` provider streams over LinkPlay, which the WiiM reports as **DLNA** (mode 2). Gating on `AIRPLAY` alone silently dropped WiiM-led joins (no spinner, member never added). All other Bose sources (BLUETOOTH, UPNP, AUX, etc.) use native Bose zone API (`/setZone`, `/removeZoneSlave`); WiiM can't join a native Bose zone, so a WiiM anywhere + non-MA leader = `none`.
