...
- **Bose WS half-open (2026-09-22)**: `boseWatcher` WS used to only listen, so a connection the speaker silently dropped stayed ESTABLISHED forever → no reconnect → preset 5 (WCRB) intercept dead. Now pings every 30s (Bose pongs, verified), reconnects after 75s silence, + TCP keepalive. If presets stop intercepting: check add-on log for `preset N pressed`; press via `POST :8090/key PRESET_N` to test (set volume 0 first, then stop + restore — don't wake the house).
- **LG webOS pairing lost (2026-09-08 → fixed 09-22)**: TV webOS update broke HA 2026.8.1's aiowebostv pairing (`error_pairing` / "Client key not set" instantly, no TV prompt). `media_player.lg_tv_2` sat `unavailable` → tvWatcher silently never fired for 2 weeks. Fix: HA Core 2026.9.3 (aiowebostv 0.9.2 "Fix webOS 26 pairing handshake") + delete/re-add LG integration; entity ID stayed `lg_tv_2`. Re-pair can be driven via REST: `POST /api/config/config_entries/flow {handler:"webostv"}` → `{host:"192.168.1.38"}` → `{}` (someone must accept TV prompt). Note `unavailable` is NORMAL ~30s after TV off (network sleep) — only a problem if TV is on. tvWatcher logs availability changes and skips polls with no state string (a deleted entity returned `{message}` and read as "on" → false switch).
- **MA timeouts ≠ outage (2026-09-22)**: right after an MA restart, `play_media` took >10s but still succeeded → UI flashed "Music Assistant unreachable" while music started. `maClient` now gives `play_media` 25s (`SLOW_COMMANDS`; others 10s) and timeouts reject with code `MA_TIMEOUT` + "slow to respond — it may still start". "Unreachable" = real connection failure only. `LST_MA_TIMEOUT_MS` env overrides for tests.
